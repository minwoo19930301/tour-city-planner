...
- 날짜 헤더의 중앙 ‘하루 이동코스’는 루트부터 마지막 블록까지 실제 연결된 경로를 라디오로 나열한다. 선택하면 지도와 외부 구글 맵 링크가 갱신된다. 경로가 많으면 200개씩 넘겨 볼 수 있다.
- 모든 연결 구간에 ‘가는 길’과 삭제 ×를 함께 표시한다.
- 추가/이동 시간은 앞뒤 시각의 평균(분 단위)이며 옆 배치는 이웃 시각을 사용한다. 한쪽만 있으면 ±1시간, 양쪽 모두 미정이면 12:00을 제안한다. 편집에서 ‘미정 · 순서만’으로 지울 수 있다.

### 배치와 길찾기 (2026-09-24)
- 새 일정과 기존 일정 드래그 모두 각 카드의 앞·뒤·양옆 배치 영역을 사용합니다. 나란한 일정은 최대 3개입니다.
- 시간을 직접 수정하면 해당 일정이 시간에 맞는 행으로 이동하고 연결선도 재구성됩니다. 드래그 시에는 새 위치 기준으로 시간을 채웁니다.
- 구간 지도는 대중교통, 하루 코스는 자동차가 기본이며 이동 수단을 전환할 수 있습니다. 대중교통은 구간을 선택해서 조회합니다.
- 하루 코스는 화면 하단의 큰 지도 패널에서 후보 경로를 선택합니다. Google Maps의 전체 기능은 지도 열기 링크로 이용합니다.

### 시작 화면·AI 프롬프트 (2026-09-24)
- 국가 선택 목록을 화면 최상위에 배치하여 모바일 위치 어긋남을 수정했습니다.
- 대표 도시 포함 최대 5개를 표시합니다. 한국인 출국 국가 순위와 주요 직항 노선 이용 통계를 참고한 편집 순서이며, 모든 도시의 실시간 여객 순위를 의미하지 않습니다.
- 참고: 연합뉴스 2025-12-26, https://www.yna.co.kr/view/AKR20251224094000518 (법무부 내국인 출국 통계 및 국토교통부 항공통계 인용).
- 시작 화면의 ‘일정 추가하기’를 제거했습니다. 국가·날짜를 선택하고 바로 시작합니다.
- 하루 코스 선택 시 해당 항목을 목록 상단으로 스크롤합니다.
- AI 프롬프트는 현재 v4 JSON, 실제 지원 아이콘, 분기 id/row/edge 규칙을 안내합니다. 한글 JSON의 UTF-8 Base64URL 인코딩→디코딩→일정 로더 테스트를 통과했습니다. 외부 AI가 생성하는 모든 답변의 정확성을 보증하는 검증은 아닙니다.
